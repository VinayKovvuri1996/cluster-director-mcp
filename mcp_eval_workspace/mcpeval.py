...
def run_one_testcase(one_testcase: Any, index: int) -> Dict[str, Any]:
    ...
    result_report = {
        ...
        "match_name": False,
        "match_args": False,
        "omitted_result_display": False, # Tracks if we intentionally skipped this check
        "error_msg": None,
        "duration": 0.0
    }
    ...
    try:
        # Added timeout=60 to prevent the invisible prompt freezing
        subprocess.run(command, shell=True, check=False, capture_output=True, text=True, cwd=get_working_directory(), timeout=120)
        
        end_time = time.time()
        result_report["duration"] = round(end_time - start_time, 2)

        if not check_file_exists(log_file_stderr):
            result_report["error_msg"] = "Log file not generated."
            return result_report

        golden_response = one_testcase.get("response", {})
        if golden_response.get("name"):
            parsed_results_dict = extract_tool_call.parse_log(log_file_stderr)
            if parsed_results_dict:
                
                # Graceful ResultDisplay check
                golden_result_display = golden_response.get("resultDisplay")
                if golden_result_display is not None:
                    if compare_test_results(parsed_results_dict.get("resultDisplay"), golden_result_display):
                        result_report["score"] += 0.5
                        result_report["match_result_display"] = True
                else:
                    # If omitted from golden JSON, auto-pass it to evaluate Intent only
                    result_report["score"] += 0.5
                    result_report["match_result_display"] = True
                    result_report["omitted_result_display"] = True
                ...
                if parsed_results_dict.get("name") == golden_response.get("name"):
                    result_report["score"] += 0.15
                    result_report["match_name"] = True
                        
                # Compare Arguments (0.15 pts)
                if parsed_results_dict.get("args") == golden_response.get("args"):
                    result_report["score"] += 0.15
        ...
        end_time = time.time()
        result_report["duration"] = round(end_time - start_time, 2)
        result_report["error_msg"] = "Timeout Exceeded (60s). The MCP server or CLI hung."
        return result_report        
...
def print_diagnostic_report(results: List[Dict[str, Any]], total_runtime: float):
    print("\n" + "="*80)
    ...
    for i, r in enumerate(results, 1):
        ...
        if r['score'] == 1.0:
            ...
            note = "[Intent Eval]" if r.get('omitted_result_display') else ""
            print(f" [Test {i}] PASSED {time_str} {note}")
            continue
            
        print(f"\n [Test {i}] FAILED ({r['score']:.2f} / 1.0) ({r['duration']}s)")
...
def main(argv: Sequence[str]) -> None:
    ...
    print_diagnostic_report(all_results, total_runtime)
